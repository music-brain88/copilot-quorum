...
<!-- LLM Context: Quorum は copilot-quorum のコア概念。Discussion は 3 フェーズ（Initial Query → Peer Review → Synthesis）の議論プロセス。Consensus は Vote + QuorumRule による承認/却下メカニズム。OrchestrationStrategy は domain の enum（Quorum/Debate の variant、domain/src/orchestration/strategy.rs）。それを実行する StrategyExecutor trait は application 層(application/src/use_cases/run_quorum/strategy_executor.rs、実装は同ディレクトリの quorum_strategy.rs / debate_strategy.rs)にあり、RunQuorumUseCase(同ディレクトリの mod.rs)が exhaustive match でディスパッチする(#314)。主要ファイルは domain/src/quorum/（vote.rs, rule.rs, consensus.rs）、domain/src/orchestration/strategy.rs（OrchestrationStrategy enum）、application/src/use_cases/run_quorum/（RunQuorumUseCase + StrategyExecutor 実装群）。 -->
