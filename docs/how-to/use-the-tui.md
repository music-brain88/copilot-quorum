...
| `gt` | 次のタブ |
| `gT` | 前のタブ |
| `?` | ヘルプ表示（`j`/`k`/`↓`/`↑` スクロール、`g`/`G` 先頭/末尾、`?`/`Esc` で閉じる） |
| `Ctrl+C` | 終了 |
...
| `:q` / `:quit` | 複数タブ時はアクティブタブを閉じる。最後の 1 枚ではアプリ終了（Vim 準拠） |
| `:qa` / `:qall` | 全タブを閉じてアプリ終了 |
...
| `:help` | ヘルプを表示 |
| `:q` / `:quit` | 複数タブ時はアクティブタブを閉じる。最後の 1 枚で終了 |
| `:qa` / `:qall` / `:exit` | アプリ全体を終了 |
...
### 同一タブへの再入力（Cancel & Replace）

タスク実行中のタブに新しいリクエストを送ると（例: Agent が作業中のタブでさらに何か入力する）、
実行中のタスクは自動的にキャンセルされ、完了後に新しいリクエストへ差し替わります
（issue [#212](https://github.com/music-brain88/copilot-quorum/issues/212)）。
差し替え待ちの間は Flash メッセージで「実行中のタスクをキャンセル中…」と通知されます。
Agent form のタスクがキャンセルされた場合、途中までの進捗（完了/失敗したタスクや直近の思考）
が要約され、差し替え後のリクエストの先頭に自動的に補足されます。

**既知の制限**: 即座にキャンセルされるのは実行中のタスクが Agent form の場合だけです。
Ask / Discuss / Review が実行中のタブに再入力すると、差し替えリクエストは「キャンセル」
ではなく実行中のタスクが自然に完了するまで待たされます（キャンセルトークンが
Agent 以外の use case に配線されていないため）。この間もタブは操作可能で、完了後は
自動的に差し替えリクエストが実行されます。
